package com.vladproduction.ch18_conditions_concrete;
...
public final class BoundedBufferWithManyConditions<T> {
    ...
    //need to condition for our while loops, concrete condition for buffer when empty or full
    private final Condition notFull;
    private final Condition notEmpty;

    @SuppressWarnings("unchecked")
    public BoundedBufferWithManyConditions(int capacity) {
        elements = (T[]) new Object[capacity];
        lock = new ReentrantLock();
        notFull = lock.newCondition(); // get a condition object for the lock if buffer is not full
        notEmpty = lock.newCondition(); // get a condition object for the lock if buffer is not empty
    }
    ...
    public void put(T element) {
        ...
        try {
            ...
            notEmpty.signal(); //signaling we are available for take some elements from the buffer
        }
    ...
    public T take() {
        ...
        try{
            ...
            notFull.signal(); //signaling we are available for put some elements to the buffer
            return element;
        }
        catch (InterruptedException e) {
            Thread.currentThread().interrupt();
